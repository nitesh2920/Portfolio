...
import { FaAward, FaUsers, FaBriefcase, FaGithub, FaTrophy, FaLaptopCode } from 'react-icons/fa';
...
const Experience = () => {
  ...
  const getIcon = (title, isWorkExperience = false) => {
    const lowerTitle = title.toLowerCase();
    const iconSize = "w-5 h-5 sm:w-6 sm:h-6"; 
    
    if (isWorkExperience) {
      if (lowerTitle.includes("frontend") || lowerTitle.includes("developer")) return <FaLaptopCode className={`text-accent-1 ${iconSize}`} />;
      if (lowerTitle.includes("web")) return <FaBriefcase className={`text-accent-1 ${iconSize}`} />;
  ...
  const renderTimelineSection = (items, isWorkExperience = false, startIndex = 0) => {
    return items.map((item, index) => (
      <motion.div
        key={`${isWorkExperience ? 'work' : 'achievement'}-${index}`}
      ...
      >
        ...
        >
          <div className="relative z-10">
            {getIcon(item.role, isWorkExperience)}
          </div>
        </motion.div>
    ...
    <section id="experience" className="py-20 bg-secondary-bg relative overflow-hidden" ref={containerRef}>
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="relative max-w-6xl mx-auto mt-16">
          ...
          {/* Timeline dots - Fixed size to be proportional to timeline pipe */}
          <div className="absolute left-[calc(2rem-2px)] sm:left-[calc(2.5rem-2px)] top-0 h-full">
            {[...workExperience, ...achievementsAndLeadership].map((_, index) => (
              <motion.div
                key={index}
                className="absolute w-1.5 h-1.5 bg-accent-1 rounded-full border border-primary-bg shadow-sm"
                style={{ top: `${(index + 1) * (100 / ([...workExperience, ...achievementsAndLeadership].length + 1))}%` }}
                initial={{ scale: 0, opacity: 0 }}
                whileInView={{ 
                  scale: 1, 
                  opacity: 1,
                  boxShadow: "0 0 8px rgba(0, 245, 195, 0.4)"
                }}
                transition={{ 
                  delay: index * 0.2 + 0.5,
                  duration: 0.5,
                ...
                whileHover={{ 
                  scale: 2,
                  boxShadow: "0 0 12px rgba(0, 245, 195, 0.6)"
                }}
              />
            ))}
          </div>
          ...
          <div className="mb-16">
            <motion.h2 
              className="text-2xl sm:text-3xl font-bold text-accent-1 font-mono mb-8 flex items-center gap-3 ml-20 sm:ml-24"
              initial={{ opacity: 0, x: -20 }}
              whileInView={{ opacity: 1, x: 0 }}
              viewport={{ once: true }}
            >
              <FaLaptopCode className="text-accent-2" />
              Work Experience
            </motion.h2>
            {renderTimelineSection(workExperience, true, 0)}
          </div>
          ...
          <motion.div 
            ...
            viewport={{ once: true }}
          >
            <div className="absolute left-20 sm:left-24 right-0 h-px bg-gradient-to-r from-accent-1/30 via-accent-2/20 to-transparent"></div>
          </motion.div>
          ...
          <div>
            <motion.h2 
              ...
              viewport={{ once: true }}
            >
              <FaTrophy className="text-accent-2" />
              Achievements & Leadership
            </motion.h2>
            {renderTimelineSection(achievementsAndLeadership, false, workExperience.length)}
          </div>
        </div>
      </div>
    </section>
